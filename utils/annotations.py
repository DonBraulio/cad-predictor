...
def unify_annotations(ann_list: List[Annotation]) -> Annotation:
    """
    Given a list like: [an1, an2, an3], get new Annotations
    with segments that are present in at least 2 annotations of the list.
    For each label, this means:
    result = (an1 & an2) | (an2 & an3) | (an1 & an3)
    This can actually handle any number of annotations, but the voting
    will always require at least 2 coincident labels.
    """
    pair_matches = {}
    # Iterate over each combination of an1, an2 (hopefully, 3)
    for idx, an1 in enumerate(ann_list[:-1]):
        for an2 in ann_list[idx + 1 :]:
            for label in an1.labels():
                if label not in an2.labels():
                    continue
                t1 = an1.label_timeline(label, copy=False)
                t2 = an2.label_timeline(label, copy=False)
                if label not in pair_matches:
                    pair_matches[label] = []

                # Get intersections for this pair of annotations
                t_intersect = Timeline()
                for s1, s2 in t1.co_iter(t2):
                    t_intersect.add(s1 & s2)
                pair_matches[label].append(t_intersect)

    result = Annotation()
    for label in pair_matches:
        t_label = Timeline()

        # Get union of intersecting pairs
        for match in pair_matches[label]:
            t_label |= match

        # Remove redundant overlapping segments and add to label
        for segment in t_label.support_iter():
